...
python3 .trellis/scripts/fusion/checkpoint.py \
  ...
  --next "run execute-plan-tdd from Slice 1 with slice budget 1"
```
...
Next step: run $execute-plan-tdd from Slice 1 with a slice budget of 1, keeping tests red before green. Only increase the budget if the user explicitly wants a larger run.
```
